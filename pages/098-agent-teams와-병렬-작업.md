...
병렬 작업 전에 공통 계약을 짧게라도 문서화해야 합니다.

```text
Shared contract:
- Endpoint: PATCH /api/profile
- Request: { displayName: string, bio?: string }
- Response: { id: string, displayName: string, bio: string | null }
- Error: 400 validation_error, 401 unauthenticated, 403 forbidden
- UI copy source: docs/profile-copy.md
```

계약이 없으면 각 에이전트가 그럴듯한 타입과 오류 형식을 따로 만들고, 마지막 통합 단계에서 비용이 커집니다.
...
병렬 작업은 결과가 늦게 모이면 충돌을 늦게 발견합니다. 각 에이전트에게 중간 보고 형식을 맞추게 하세요.
...
특히 `Contract changes`가 비어 있지 않다면 다른 에이전트에게 즉시 공유해야 합니다.
...
- [ ] 병합 순서와 검증 계획이 있다.
- [ ] 결과 통합 담당이 명확하다.
- [ ] 중간 보고에 계약 변경과 차단 사항이 포함된다.
- [ ] 통합 담당자가 최종 판단을 한 번 더 한다.
